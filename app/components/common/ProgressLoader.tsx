...
const ProgressLoader = ({ progress }: { progress: number }) => {
  ...
  return (
    ...
    <div
      className={`fixed top-0 left-0 w-full h-full flex items-center justify-center pointer-events-none transition-opacity duration-1000 ${progress === 100 ? 'opacity-0' : 'opacity-100'}`}
      style={{ padding: '1rem' }}
    >
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
        <h1 className="text-4xl md:text-5xl lg:text-6xl text-white tracking-[0.2em] font-medium uppercase text-center" style={{ fontFamily: 'soria' }}>Devesh Dev Fauzdar</h1>
        <p className="text-sm md:text-base text-gray-400 tracking-[0.3em] font-light uppercase text-center">creative frontend developer</p>
      </div>
      <svg
